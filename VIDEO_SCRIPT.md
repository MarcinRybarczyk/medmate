...
Target: <= 5:00, 1280x720. Frames are rendered by `make_video.py`, an English
**voiceover** is generated by `scripts/gen_tts.py` (edge-tts, voice
`en-US-AndrewNeural`, rate -5%), and `scripts/assemble_av.py` times each frame to
its narration (+0.5 s buffer) and writes the ffmpeg concat list + `medmate_demo.srt`.
ffmpeg muxes H.264 video + AAC audio. Final: ~2:12, well under 5:00. The on-screen
captions below double as the subtitle track for accessibility / muted playback.
